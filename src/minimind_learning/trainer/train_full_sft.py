...
# 代码和train_pretrain.py完全一样,出了Dataloader和batch_size超参数不一样. (batchsize)减小(32->16)是因为sft可能会有更大的模型输入.另外sft之前训练过 所以sft更小的初始学习率(5e-4 -> 5e-7).

def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    '''
    和train_pretrain里面的完全一样
    '''
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            lm_checkpoint(lm_config, weight=args.save_weight, model=model, optimizer=optimizer, 
                         epoch=epoch, step=step, wandb=wandb, save_dir=args.ckpt_dir, scaler=scaler)
            model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../datasets/sft_mini_512.jsonl", help="训练数据路径")
