...
也就是说，后期很多改进并不是在回答“LLM 是什么”，而是在回答“同样是 attention-based 模型，怎样让它更便宜、更稳、更长、更适合落地”。

如果从这个角度看，后面的这些主题其实就比较容易分清了：
...
- 它们大多没有改变语言模型最基本的输入输出形式
- 它们也没有推翻 attention + FFN 这条主干
- 真正被反复打磨的，是 attention 周围最贵、最慢、最吃显存、最影响长上下文的那些部分

所以如果只保留一个最重要的结论，我会更倾向于这样记：

- 模型后期改进，很多时候不是在重新定义模型，而是在持续优化 attention 的位置建模、计算效率、缓存机制和长上下文能力
